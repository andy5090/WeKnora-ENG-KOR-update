...
interface Props {
  ...
  disabled?: boolean
  placeholder?: string
  // Optional: All model list passed from outside, if provided then don't call API
...
// Listen to allModels changes, automatically filter models of current type
...
// Load model list (only called when allModels not provided)
const loadModels = async () => {
  // If allModels provided externally, no need to load
  if (props.allModels) {
    return
  }
  ...
  try {
    ...
    // Filter models by type on frontend
...
const handleModelChange = (value: string) => {
  // If add model option selected, trigger add event without updating selected value
...
onMounted(() => {
  // Only load if allModels not provided
  if (!props.allModels) {
    loadModels()
  }
})
