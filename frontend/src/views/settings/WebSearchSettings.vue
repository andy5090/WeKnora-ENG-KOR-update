...
const isInitializing = ref(true) // Flag to mark if initializing, disable auto-save during initialization
const initialConfig = ref<WebSearchConfig | null>(null) // Save initial configuration for comparison
...
const loadTenantConfig = async () => {
  try {
    ...
    if (response.data) {
      ...
      // Save a copy of initial configuration (for later comparison)
      ...
      // API key is hidden in response, if "***", it means configured but actual value not returned
      ...
      // If no configuration data, save default configuration
      initialConfig.value = {
        provider: '',
        api_key: '',
        max_results: 5,
    ...
    // If configuration doesn't exist, use default values (don't show error)
    initialConfig.value = {
      provider: '',
      api_key: '',
      max_results: 5,
...
const hasConfigChanged = (): boolean => {
  if (!initialConfig.value) {
    return true // If no initial configuration, consider it changed
  }
  ...
  // Compare if configuration has changed (ignore API key '***' placeholder)
...
const saveConfig = async () => {
  // If configuration hasn't changed, don't save
  if (!hasConfigChanged()) {
    return
  }
  ...
  try {
    ...
    // Update initial configuration to avoid duplicate saves
    initialConfig.value = {
      provider: config.provider,
...
onMounted(async () => {
  ...
  // loadTenantConfig internally handles isInitializing, no need to set it here
})
</script>
