...
show_help() {
    ...
    echo "  --no-pull      Don't pull images on startup (default is to pull)"
...
install_ollama() {
    ...
    if [ "$OS" = "Darwin" ]; then
        ...
        if ! command -v brew &> /dev/null; then
            # Install via package
            log_info "Homebrew not installed, using direct download..."
...
start_ollama() {
    ...
    if curl -s "http://localhost:$OLLAMA_PORT/api/tags" &> /dev/null; then
        ...
        # Note: Official recommendation is to use systemctl or launchctl to manage service, direct background run is only for temporary scenarios
...
check_docker() {
	...
	if detect_compose_cmd; then
		...
		log_error "Docker Compose not detected (neither docker compose nor docker-compose). Please install one of them."
		return 1
	fi
...
# Pre-pull sandbox image (required for Agent Skills, pull only, do not start)
ensure_sandbox_image() {
    ...
    log_info "Sandbox image ($sandbox_image) not found, pulling in background..."
    log_info "Agent Skills feature depends on this image, pull must complete before first use"

    (
        if PLATFORM=$PLATFORM "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD --profile sandbox pull sandbox 2>/dev/null; then
            log_success "Sandbox image pull completed: $sandbox_image"
        else
            log_warning "Sandbox image pull failed, Agent Skills feature may be unavailable"
            log_warning "You can pull manually later: $DOCKER_COMPOSE_BIN $DOCKER_COMPOSE_SUBCMD --profile sandbox pull sandbox"
        fi
    ) &
...
start_docker() {
    ...
    # Enter project root directory before executing docker-compose command
    cd "$PROJECT_ROOT"
...
stop_docker() {
    ...
    if [ $? -ne 0 ]; then
        ...
        log_warning "Docker environment check failed, will still try to stop containers..."
    fi
...
pull_images() {
    ...
    # Pull sandbox image (sandbox is in profile, needs separate pull)
...
check_environment() {
    ...
    if docker image inspect "$sandbox_image" &> /dev/null; then
        ...
        log_warning "Sandbox image not found: $sandbox_image (Agent Skills feature requires this image)"
        log_info "You can pull with: $0 -p or docker pull $sandbox_image"
    fi
    ...
    if docker info &> /dev/null; then
        ...
        log_warning "Cannot get container status, Docker may not be running"
    fi
...
if [ "$STOP_SERVICES" = true ]; then
    ...
    if [ "$START_OLLAMA" = true ] && [ "$START_DOCKER" = true ]; then
        if [ $OLLAMA_RESULT -eq 0 ] && [ $DOCKER_RESULT -eq 0 ]; then
            log_success "All services started, accessible at:"
            printf "%b\n" "${GREEN}  - Frontend UI: http://localhost:${FRONTEND_PORT:-80}${NC}"
            printf "%b\n" "${GREEN}  - API Interface: http://localhost:${APP_PORT:-8080}${NC}"
            printf "%b\n" "${GREEN}  - Jaeger Distributed Tracing: http://localhost:16686${NC}"
            echo ""
        ...
        log_success "Ollama service started, accessible at:"
        printf "%b\n" "${GREEN}  - Ollama API: http://localhost:$OLLAMA_PORT${NC}"
    elif [ "$START_DOCKER" = true ] && [ $DOCKER_RESULT -eq 0 ]; then
        log_success "Docker containers started, accessible at:"
        printf "%b\n" "${GREEN}  - Frontend UI: http://localhost:${FRONTEND_PORT:-80}${NC}"
        printf "%b\n" "${GREEN}  - API Interface: http://localhost:${APP_PORT:-8080}${NC}"
        printf "%b\n" "${GREEN}  - Jaeger Distributed Tracing: http://localhost:16686${NC}"
        echo ""
