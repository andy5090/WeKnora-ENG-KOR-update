<template>
  <div ref="rootElement" class="agent-stream-display">
    ...
    <div v-if="!isConversationDone && eventStream.length > 0" class="loading-indicator" :class="{ 'no-timeline': !shouldShowTimeline }">
      ...
      <!-- Option 4: Typewriter effect (commented out, can be replaced) -->
  ...
  <!-- Global overlay: unified container for Web/KB hover content -->
  <Teleport to="body">
    <div
...
// Overlay state (shared by Web/KB)
...
const intermediateStepsSummary = computed(() => {
  ...
  // Optimize connecting words to make sentences more fluent
  if (parts.length === 1) {
    return parts[0];
  } else if (parts.length === 2) {
    return `${parts[0]}, ${parts[1]}`;
  } else {
    // 3 or more: use commas for the first few, comma for the last
...
// KB citations: hover to show summary in overlay; click to navigate to KB details
type KbTooltipState = {
  loading: boolean;
  error?: string;
  html?: string;
...
const getToolSummary = (event: any): string => {
  ...
  if (toolName === 'search_knowledge' || toolName === 'knowledge_search') {
    ...
    if (toolData?.fetched_chunks !== undefined) {
      ...
      return `View ${toolData.fetched_chunks}/${toolData.total_chunks ?? '?'} chunks of ${title}`;
    }
  } else if (toolName === 'todo_write') {
...
const isBookIcon = (toolName: string): boolean => {
  return false; // No longer use t-icon's book, switched to SVG icon
};
...
const getToolDescription = (event: any): string => {
  ...
  if (toolName === 'search_knowledge' || toolName === 'knowledge_search') {
    return success ? 'Search Knowledge Base' : 'Search Knowledge Base Failed';
  } else if (toolName === 'web_search') {
    return success ? 'Web Search' : 'Web Search Failed';
  } else if (toolName === 'get_document_info') {
    return success ? 'Get Document Info' : 'Get Document Info Failed';
  } else if (toolName === 'thinking') {
    return success ? 'Thinking Complete' : 'Thinking Failed';
  } else if (toolName === 'todo_write') {
    return success ? 'Update Task List' : 'Update Task List Failed';
  } else {
    const localizedName = getLocalizedToolName(toolName);
    return success ? `Call ${localizedName}` : `Call ${localizedName} Failed`;
  }
};
...
const handleAddToKnowledge = (answerEvent: any) => {
  ...
  MessagePlugin.info('Editor opened, please select knowledge base and save');
};
</script>
...
.event-item {
  ...
  // First event connection starts from node
  &:first-child::before {
    top: 14px;
  }
  
  // Last event does not show bottom connection
  &.event-last::before {
    bottom: auto;
    height: 22px;
  }
  ...
  &.no-timeline {
    ...
    // Ensure timeline for all event types is hidden (use stronger selector)
    &.event-last::before,
    &.event-last::after,
    &:first-child::before,
    &:first-child::after,
...
.thinking-event {
  ...
  .thinking-phase {
    ...
    &.thinking-last {
      ...
      // When the last Thinking is the final answer, font should be larger
      .thinking-content {
        font-size: 14px;
      }
    }
  ...
  .thinking-content {
    ...
    &.markdown-content {
      ...
      :deep(img) {
        ...
        width: auto;
        height: auto;
        min-height: 100px; /* Prevent image height collapse causing jitter during streaming output */
        border-radius: 8px;
        display: block;
        margin: 8px 0;
        ...
        background-color: #f9fafb; /* Placeholder background color during loading */
...
// Answer Event - similar to thinking but with unique styles
.answer-event {
  animation: fadeInUp 0.25s ease-out;
  min-height: 20px;
  ...
  .answer-toolbar {
    ...
    :deep(.t-button) {
      ...
      // t-button__text contains icon, needs to display but only show icon
      .t-button__text {
        display: inline-flex !important;
        align-items: center;
        justify-content: center;
