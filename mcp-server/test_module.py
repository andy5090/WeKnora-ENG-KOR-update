...
def test_environment():
    ...
    print(f"WEKNORA_BASE_URL: {base_url or 'Not set (will use default)'}")
    print(f"WEKNORA_API_KEY: {'Set' if api_key else 'Not set'}")

    if not base_url:
        print("Tip: You can set the environment variable WEKNORA_BASE_URL")

    if not api_key:
        print("Tip: It is recommended to set the environment variable WEKNORA_API_KEY")
...
def test_package_installation():
    ...
    try:
        # Check if it can be installed in development mode
        result = subprocess.run(
