<template>
  <div class="model-settings">
      ...
      <div class="builtin-models-info">
        <div class="info-box">
          ...
          <div class="info-content">
            <p>Built-in models are visible to all tenants, sensitive information will be hidden, and they cannot be edited or deleted.</p>
            <p class="doc-link">
              <t-icon name="link" class="link-icon" />
              <a href="https://github.com/Tencent/WeKnora/blob/main/docs/BUILTIN_MODELS.md" target="_blank" rel="noopener noreferrer">
                View Built-in Models Management Guide
              </a>
            </p>
          </div>
        </div>
    ...
    <div class="model-category-section" data-model-type="vllm">
      ...
      <div v-else class="empty-state">
        <p class="empty-text">No VLLM vision models available</p>
...
// Convert backend model format to legacy frontend format
function convertToLegacyFormat(model: ModelConfig) {
  return {
    id: model.id!,
    name: model.name,
    source: model.source,
    modelName: model.name,  // Display name as model name
    baseUrl: model.parameters.base_url || '',
    apiKey: model.parameters.api_key || '',
    provider: model.parameters.provider || '', // Add provider field
    dimension: model.parameters.embedding_parameters?.dimension,
    isBuiltin: model.is_builtin || false
  }
}

// Deduplication function: compare all fields except id, keep only the first one for duplicates
function deduplicateModels(models: any[]) {
  const seen = new Map<string, any>()
  
  return models.filter(model => {
    // Create a signature without id for comparison
...
const loadModels = async () => {
  loading.value = true
  try {
    // Get all models directly, without filtering by type
...
const handleModelSave = async (modelData: any) => {
  try {
    ...
    // Embedding models must fill dimension
    ...
    const apiModelData: ModelConfig = {
      ...
      parameters: {
        ...
        provider: modelData.provider || '', // Add provider field
...
// Get model operation menu options
...
const handleMenuAction = (data: { value: string }, type: 'chat' | 'embedding' | 'rerank' | 'vllm', model: any) => {
  ...
  if (value.indexOf('edit-') === 0) {
    ...
    // Use confirmation dialog for confirmation
...
// Load model list when component mounts
onMounted(() => {
  loadModels()
})
</script>
...
// Add model button style optimization
:deep(.add-model-btn) {
  display: inline-flex;
  align-items: center;
  gap: 6px;
...
:deep(.t-popup__content) {
  ...
  .t-dropdown__item {
    ...
    // Delete button special style - override default hover style
    &.t-dropdown__item--theme-error {
      color: #f56c6c;

      &:hover {
        background: #fef0f0;
        color: #e53e3e !important; // Use !important to ensure override default style
      }
