...
class HeaderTrackerHook(BaseModel):
    """Header tracking Hook configuration class, supports header recognition in multiple scenarios"""
    ...
    extract_header_fn: Callable[[Match[str]], str] = Field(
        ...
        description="Function to extract header content from start match result (defaults to entire matched content)",
    )
    priority: int = Field(default=0, description="Priority (higher priority matches first when multiple configurations exist)")
    case_sensitive: bool = Field(
        default=True, description="Whether case-sensitive (only effective when string pattern is passed)"
    )
...
# Initialize header Hook configuration (provides default config: supports Markdown tables, code blocks)
DEFAULT_CONFIGS = [
    ...
    #     priority=20,  # Code blocks have higher priority than tables
    #     case_sensitive=True,
    # ),
...
class HeaderTracker(BaseModel):
    ...
    def update(self, split: str) -> Dict[int, str]:
        ...
        # 1. Check if there are header end markers
        ...
        # 2. Check if there are new header start markers (only process inactive and not-ended ones)
        for config in self.header_hook_configs:
            if (
    ...
    def get_headers(self) -> str:
        ...
        # Sort headers by priority in descending order
