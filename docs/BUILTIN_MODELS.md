...
Built-in models are system-level model configurations that are visible to all tenants, but sensitive information is hidden and cannot be edited or deleted. Built-in models are typically used to provide system default model configurations, ensuring all tenants can use unified model services.
...
```sql
-- Example: Insert a LLM built-in model
INSERT INTO models (
    id,
    tenant_id,
    name,
    ...
    10000,                                -- Tenant ID (use the first tenant)
    'GPT-4',                              -- Model name
...
### 3. Verify Insert Result

Execute the following SQL query to verify that the built-in model was successfully inserted:
...
3. **Parameter Format**: The `parameters` field must be in valid JSON format
4. **Idempotency**: Use `ON CONFLICT (id) DO NOTHING` to ensure that repeated execution does not cause errors
...
UPDATE models 
SET is_builtin = true 
WHERE id = 'Model ID' AND name = 'Model Name';
```
