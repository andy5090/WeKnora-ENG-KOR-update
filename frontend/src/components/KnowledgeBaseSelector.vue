...
// Filter: only show initialized (has embedding & summary)
...
// helper: get real DOM element from props.anchorEl (supports multiple input forms)
const resolveAnchorEl = () => {
  const a = props.anchorEl
  if (!a) return null
  // If Vue ref: get .value
  if (typeof a === 'object' && 'value' in a) {
    return a.value ?? null
  }
  // If component instance (may have $el)
...
// Calculate dropdown position: horizontally center-aligned to button midpoint, handle viewport boundaries
const updateDropdownPosition = () => {
  ...
  if (spaceBelow >= minDropdownHeight + offsetY) {
    ...
    // Pop up, prioritize preferredHeight, only expand to maxHeight when necessary
...
watch(() => props.visible, async (v) => {
  if (v) {
    ...
    // Listen to resize/scroll for fine-tuning (use passive for better performance)
...
.kb-dropdown {
  ...
  z-index: 10000;
  margin: 0;
  /* Ensure accurate positioning, animation uses scale instead of translate */
  transform-origin: top left;
  display: flex;
  flex-direction: column;
}

/* Width controlled by JS (dropdownWidth), only internal styles here */
.kb-search {
  padding: 8px 10px;
...
.kb-list {
  ...
  overflow-y: auto;
  padding: 6px 8px;
  /* Ensure scrolling is limited to this container */
