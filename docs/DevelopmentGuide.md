...
If you need to frequently modify `app` or `frontend` code, **you don't need to rebuild Docker images every time**. You can use local development mode instead.
...
This will run the Go application locally. After modifying code, press Ctrl+C to stop and re-run.
...
```bash
# After each code modification, you need to:
...
make dev-frontend  # Frontend code changes auto hot-reload (no restart needed)
```

**Time Required**:
- First-time startup: 1-2 minutes
- Subsequent backend changes: 5-10 seconds (restart Go app)
- Subsequent frontend changes: Real-time hot reload

## Using Air for Backend Hot Reload (Optional)

If you want the backend to auto-restart after code changes, you can install `air`:
...
Create `.air.toml` in the project root directory:
...
Use VS Code or GoLand's debugging features to connect to the locally running Go application.
...
When you're done with development and need to deploy, you'll need to build images:
...
sh scripts/build_images.sh -p  # Build backend only
sh scripts/build_images.sh -f  # Build frontend only
...
### Q: Getting database connection error when starting dev-app

A: Make sure you've run `make dev-start` first and waited for all services to start (about 30 seconds).

### Q: Getting CORS error when frontend accesses API

A: Check the frontend proxy configuration and ensure `vite.config.ts` has the correct proxy settings.

### Q: What if DocReader service needs to be rebuilt?

A: DocReader still uses Docker images. If you need to modify it, you need to rebuild:
