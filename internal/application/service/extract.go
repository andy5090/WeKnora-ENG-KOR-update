...
4. **Business Scenarios**: What business analysis or application scenarios could this table be used for?
5. **Key Features**: What are the notable characteristics of the data? (e.g., contains geographic locations, has category labels, has hierarchical relationships, etc.)

**Important Notes**:
- Do not output specific data values or sample content
- Use generalized descriptions so users can quickly determine if this table contains the information they need
...
4. **Data Characteristics**: Notable features of the data (e.g., unique identifier, nullable, has enumeration values, has units, etc.)
...
**Important Notes**:
- Do not output specific data values, only describe the metadata of fields
- Use clear business terminology for easy understanding and searching
- If enumeration value ranges can be inferred from sample data, summarize them (e.g., status field contains pending/in-progress/completed statuses)`
)
...
func (s *DataTableSummaryService) generateColumnDescriptions(ctx context.Context, chatModel chat.Chat, tableName, schemaDesc, sampleDesc string) (string, error) {
	...
	return fmt.Sprintf("# Table Column Info\n\nTable Name: %s\n\n%s", tableName, response.Content), nil
}
...
func (s *DataTableSummaryService) buildSampleDataDescription(sampleData *types.ToolResult, maxRows int) string {
	...
	builder.WriteString(fmt.Sprintf("First %d rows sample data:\n", maxRows))
