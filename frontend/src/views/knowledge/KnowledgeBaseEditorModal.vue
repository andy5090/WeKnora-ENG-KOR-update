...
const validateForm = (): boolean => {
  ...
  // Validate model configuration - must configure embedding and summary models
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.mode === 'create') {
      ...
      // 2. Update complete configuration (models, chunking, multimodal, knowledge graph, etc.)
...
watch(() => props.visible, async (newVal) => {
  if (newVal) {
    // When opening popup, first reset state
    resetState()
    
    // Check if there is initial section, if yes then navigate
    ...
    // When closing popup, delay reset state (wait for animation to end)
    setTimeout(() => {
      resetState()
...
// Watch global settings popup close to refresh model list
watch(
...
// Reuse create knowledge base styles
.settings-overlay {
  position: fixed;
  top: 0;
  left: 0;
...
// Radio-group style optimization, consistent with project theme style
:deep(.t-radio-group) {
