...
const updatePosition = async () => {
  ...
  // Check again to ensure DOM is rendered
  ...
  if (top < padding) {
    // If insufficient space above, display below instead
    if (placement === 'top') {
      top = rect.bottom + 8
    } else {
      top = padding
...
.faq-tag-wrapper {
  ...
  // Ensure internal tag can shrink correctly
  :deep(.t-tag) {
...
.faq-tag-tooltip {
  ...
  // All types use unified regular border color
  &.tooltip-answer,
  &.tooltip-similar,
  &.tooltip-negative {
    // Border and arrow colors are already defined in main style as #e7ebf0
    // No need for additional override
  }
}
