<template>
    <div class="bot_msg">
        ...
        <!-- Only show traditional markdown rendering in non-Agent mode -->
        <div ref="parentMd" v-if="!session.hideContent && !session.isAgentMode">
            <!-- Directly render complete content to avoid issues caused by splitting, style consistent with thinking -->
            <!-- Only show wrapper when there is actual content -->
...
marked.use({
    mangle: false,
    headerIds: false,
    breaks: true,  // Globally enable single line break support
});
...
const markdownTokens = computed(() => {
    ...
    // First perform security processing on Markdown content
...
// Computed property: determine if there is actual content (non-empty and not just whitespace)
...
const renderToken = (token) => {
    try {
        ...
        // Use DOMPurify for final security cleanup
...
const handleCopyAnswer = async () => {
    ...
    if (!content) {
        MessagePlugin.warning(t('chat.emptyContentWarning') || 'Current answer is empty, cannot copy');
        return;
    }
...
const handleAddToKnowledge = () => {
    const content = getActualContent();
    if (!content) {
        MessagePlugin.warning(t('chat.emptyContentWarning') || 'Current answer is empty, cannot save to knowledge base');
        return;
    }
    ...
    MessagePlugin.info(t('chat.editorOpened') || 'Editor opened, please select knowledge base and save');
};

// Handle image click events in markdown-content
...
onMounted(async () => {
    // Add click events for images in markdown-content
    nextTick(() => {
        if (parentMd.value) {
...
// Content wrapper - consistent with Agent mode answer styles
.content-wrapper {
    background: #ffffff;
    border-radius: 6px;
    padding: 8px 12px;
...
// Copy and add to knowledge base button toolbar
.answer-toolbar {
    display: flex;
