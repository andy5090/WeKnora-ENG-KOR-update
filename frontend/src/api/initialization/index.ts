...
// Simplified knowledge base configuration update interface (only pass model IDs)
...
export function updateKBConfig(kbId: string, config: KBModelConfigRequest): Promise<any> {
    return new Promise((resolve, reject) => {
        console.log('Starting knowledge base configuration update (simplified)...', kbId, config);
...
// Execute configuration update by knowledge base ID (legacy version, kept for compatibility)
...
// List installed Ollama models (detailed information)
...
export function fabriText(request: FabriTextRequest): Promise<FabriTextResponse> {
    return new Promise((resolve, reject) => {
        post('/api/v1/initialization/extract/fabri-text', request)
            ...
            .catch((error: any) => {
                console.error('Failed to generate text content:', error);
                reject(error);
            });
    });
}
...
// Model provider information type
export interface ModelProviderOption {
    value: string;        // provider identifier
    label: string;        // display name
    description: string;  // description
    defaultUrls: Record<string, string>;  // Default URLs by model type
...
export function listModelProviders(modelType?: string): Promise<ModelProviderOption[]> {
    return new Promise((resolve, reject) => {
        ...
        get(url)
            ...
            .catch((error: any) => {
                console.error('Failed to get model provider list:', error);
                resolve([]); // Return empty array on failure, frontend can fall back to default
            });
    });
}