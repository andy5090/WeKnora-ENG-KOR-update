...
// User name initial (for display when no avatar)
...
// Quick navigation to specific section of settings
const handleQuickNav = (section: string) => {
  menuVisible.value = false
  uiStore.openSettings()
  router.push('/platform/settings')
  
  // Delay a bit to ensure settings page has rendered
  setTimeout(() => {
...
const loadUserInfo = async () => {
  try {
    ...
    if (response.success && response.data && response.data.user) {
      ...
      // If tenant information returned, also update tenant information
